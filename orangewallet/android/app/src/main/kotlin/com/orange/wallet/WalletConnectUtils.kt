...
class WalletConnectUtils(address: String, uri: String, chainId: String, context: Context) :
        MultiDexApplication() {
    ...
    private fun initSessionStorage() {
        storage =
                FileWCSessionStore(
                        File(cacheDir, "session_store.json").apply { createNewFile() }, moshi)
    }
    fun approveSession() {
        client = OkHttpClient.Builder().build()
        moshi = Moshi.Builder().build()
        ...
        session =
                WCSession(
                        ...
                        Session.PeerMeta(name = "OrangeWallet"))
